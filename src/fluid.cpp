	...
	if (!ImGui::GetIO().WantCaptureMouse) {
		if (button == GLFW_MOUSE_BUTTON_RIGHT) {
			if (action == GLFW_PRESS) {
				...
				getIJ(x, y, i, j);
				//cout << "row : " << i << "  col : " << j << endl; debugging cells
				if (0 < i && i <= rows && 0 < j && j <= cols) {
					if (tpSrc[i][j] == 0)
						tpSrc[i][j] = srcTemp;
					else
...
int main(void) {
	...
	for (size_t i = 0; i <= cols; ++i) {
		frameVertices[k++] = Vertex((float)i * cellSize, (float)0, 0.1f, 0.1f, 0.1f, c);
		frameVertices[k++] = Vertex((float)i * cellSize, (float)frameHeight, 0.1f, 0.1f, 0.1f, c);
	}
	for (size_t i = 0; i <= rows; ++i) {
		frameVertices[k++] = Vertex((float)0, (float)i * cellSize, 0.1f, 0.1f, 0.1f, c);
		frameVertices[k++] = Vertex((float)frameWidth, (float)i * cellSize, 0.1f, 0.1f, 0.1f, c);
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		Buffer particleBuffer(attrSizes, (GLfloat*)particleVertices, sizeof(particleVertices)); //displaying particles
		...
		particleBuffer.draw(GL_POINTS); //displaying particles
		...
		//display particles
		//displayParticles(winWidth, winHeight);
	}
